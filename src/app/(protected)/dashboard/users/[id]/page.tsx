...
export default function EditUserPage() {
  ...
  const { useLazyCheckLockStatus } = useUser();
  const [triggerCheckLockStatus] = useLazyCheckLockStatus();
  ...
  const handleCheckLockStatus = async () => {
    ...
    try {
      ...
      const data = await triggerCheckLockStatus(user.email).unwrap();

      if (data?.data) {
        alert(`User ${user.email} is ${data.data.isLocked ? '🔒 Locked' : '🔓 Unlocked'}`);
      }
    } catch (err: any) {
      alert(err?.data?.message || err?.message || 'Failed to check lock status');
    } finally {
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-6">
            ...
            <p className="text-gray-600 mb-6">
              Are you sure you want to delete the user <span className="font-semibold text-gray-900">"{user.name || user.email}"</span>?
